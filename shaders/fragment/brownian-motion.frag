...
vec2 getParticlePos(float id) {
  ...
  vec2 startPos = vec2(0.2 + 0.6 * hash(seed), 0.2 + 0.6 * hash(seed + vec2(1.0)));

  // Random walk with time (controlled by speed)
  float walkScale = 0.15;
  vec2 offset = vec2(sin(u_time * 0.8 * u_speed + id * 2.1) * hash(seed + vec2(2.0)) * walkScale, cos(u_time * 1.1 * u_speed + id * 1.7) * hash(seed + vec2(3.0)) * walkScale);

  // Add noise-based jitter for Brownian motion (controlled by jitter amount)
  offset += vec2(hash(seed + vec2(u_time * 0.5 * u_speed)) - 0.5, hash(seed + vec2(u_time * 0.7 * u_speed)) - 0.5) * u_jitterAmount;
...
void main() {
  ...
  for(int i = 0; i < 20; i++) {
    ...
    for(int t = 1; t < 15; t++) {
      ...
      float walkScale = 0.15;
      vec2 trailOffset = vec2(sin(trailTime * 0.8 * u_speed + particleId * 2.1) * hash(seed + vec2(2.0)) * walkScale, cos(trailTime * 1.1 * u_speed + particleId * 1.7) * hash(seed + vec2(3.0)) * walkScale);

      trailOffset += vec2(hash(seed + vec2(trailTime * 0.5 * u_speed)) - 0.5, hash(seed + vec2(trailTime * 0.7 * u_speed)) - 0.5) * u_jitterAmount;
