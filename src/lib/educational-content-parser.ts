...
export function parseEducationalContent(markdownContent: string): EducationalContent {
  ...
  // Parse Layer 1 - extract content between Layer 1 and Layer 2
  const layer1Section = markdownContent.match(/## Layer 1:.*?\n\n([\s\S]*?)(?=## Layer 2|$)/)
  const intuitive = {
    title: 'What is this?',
    content: layer1Section ? layer1Section[1].trim() : 'Content not found'
  }
  
  // Parse Layer 2 - extract content between Layer 2 and Layer 3
  const layer2Section = markdownContent.match(/## Layer 2:.*?\n\n([\s\S]*?)(?=## Layer 3|$)/)
  const conceptual = {
    title: 'How does this work?',
    content: layer2Section ? layer2Section[1].trim() : 'Content not found'
  }
  
  // Parse Layer 3 - extract content from Layer 3 to end
  const layer3Section = markdownContent.match(/## Layer 3:.*?\n\n([\s\S]*?)$/)
  const technical = {
    title: 'Show me the code',
    content: layer3Section ? layer3Section[1].trim() : 'Content not found'
  }
