...
// AIDEV-NOTE: Halvorsen attractor using reusable ThreeJSCanvas system
...
import { calculateHalvorsenPoint } from '@/lib/math/halvorsen'
...
function HalvorsenPoints({ controls }: { controls: HalvorsenControls }) {
  const pointsRef = useRef<THREE.Points>(null)
  const particlesRef = useRef<{ x: number; y: number; z: number }[]>([])
  
  // Initialize particles with Halvorsen-appropriate initial conditions
  const particles = useMemo(() => {
    const particles = []
    for (let i = 0; i < controls.particleCount; i++) {
      // Use smaller initial conditions closer to origin for stability
      particles.push({
        x: (Math.random() - 0.5) * 0.2,  // Small range around origin: [-0.1, 0.1]
        y: (Math.random() - 0.5) * 0.2,  // Small range around origin: [-0.1, 0.1]
        z: (Math.random() - 0.5) * 0.2,  // Small range around origin: [-0.1, 0.1]
      })
    }
    return particles
  ...
  useEffect(() => {
    if (controls.useCustomShader) {
      createThreeJSShaderMaterial('halvorsen-particles', 'halvorsen-particles', {
        ...
        console.warn('Failed to load Halvorsen shader material:', err)
      })
    } else {
  ...
  useFrame(({ clock }) => {
    ...
    // Smaller time step for Halvorsen stability vs performance
    const dt = 0.003 * controls.speed
    const positions = pointsRef.current.geometry.attributes.position.array as Float32Array
    
    // Batch update particles for better performance
    const particles = particlesRef.current
    const particleCount = particles.length
    const scale = 0.1  // Halvorsen attractor scaling - much smaller for proper visibility
    
    // Use efficient loop with minimal allocations
    for (let i = 0; i < particleCount; i++) {
      const p = particles[i]
      
      // Calculate Halvorsen equations inline for better performance
      const { newX, newY, newZ } = calculateHalvorsenPoint(
        p.x, p.y, p.z, 
        controls.a, 
        dt
      )
...
const HalvorsenAttractorGenerator: React.FC<PatternGeneratorProps> = ({ 
  width, 
  height, 
  className = "",
  controlValues 
  ...
  const controls: HalvorsenControls = useMemo(() => ({
      ...
      speed: (controlValues?.speed as number) ?? 0.8
    }), [controlValues])
  ...
  return (
    ...
    >
      <HalvorsenPoints controls={controls} />
      {controls.showAxes && (
        <AxesHelper3D 
          size={1.5} 
          opacity={0.4}
...
export default HalvorsenAttractorGenerator